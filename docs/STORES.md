...
Pricing is zero in all selected markets, age ratings are complete after the publisher's explicit IARC agreement, and runFullTrust is explained as the ordinary Win32 desktop host. FullTrust does not mean administrator privileges. The tested package and completed listing were submitted for certification on 12 September; the public Microsoft Store listing was verified on 13 September. The original CI candidate remains unsigned; Microsoft handles signing for Store distribution. The Store-delivered package version, signature and installation have not been independently inspected. Default-app association choice, multi-file activation, folders, printing, upgrades, screen-reader usability and a machine missing WebView2 remain separate test coverage gaps. State those limits accurately.
