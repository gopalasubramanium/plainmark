...
The publisher needs an active [Apple Developer account](https://developer.apple.com/account/) and a valid Developer ID Application identity. A Developer ID Application certificate has been issued and passes the local macOS code-signing certificate policy. CI credentials and notarization are still pending; this certificate check does not make the existing preview downloads signed or notarized.
