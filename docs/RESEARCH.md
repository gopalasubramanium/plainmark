...
The 0.3 changes preserve checked states in pasted HTML task lists. They also route documents containing reference definitions, footnotes or wiki links to Source view, with a reason visible to the writer. Code examples and frontmatter containing similar strings are excluded from this detection. Reference links can still render in preview; specialist footnote and wiki-link rendering is not implemented. This is an explicit compatibility boundary, not a claim that all Markdown dialects are supported.
...
Plainmark 0.3 adds both clipboard image insertion and an Image from file command. The desktop app asks to save an untitled note first, then stores each raster attachment under an `assets` folder beside the document. Unique names avoid overwriting existing pictures. Image import rejects non-image signatures, oversized input, invalid decoding in the editor and an assets directory that redirects through a symlink. The operation does not rewrite the saved Markdown file automatically; the insertion remains an ordinary unsaved edit until Save.
...
macOS distribution outside the App Store requires the appropriate Developer ID identity and notarization process. The v0.3.3 draft now has verified Developer ID signatures and Apple notarization for both Mac architectures. The workflow checks the publisher team, hardened runtime and stapled app ticket, then notarizes and verifies the outer DMG separately. Freshly downloaded archives, DMGs and mounted apps passed local trust checks. GitHub signing credentials are restricted to a protected environment with maintainer approval for each run; they are not app runtime dependencies.[^21][^22]
...
| --- | --- | --- |
| macOS Apple Silicon and Intel | Developer ID signatures; app and DMG notarization, stapled tickets and Gatekeeper checks; mounted/downloaded contents verified; Apple Silicon launch | Release remains a draft; physical Intel installation and broader macOS-version testing |
...
The research has produced working changes, not only a feature wishlist. Existing 0.2.2 functionality includes folder workspaces, tabs, visual editing, local Mermaid/math/SVG, explicit HTML execution, block synchronization and Open with integration. The 0.3 changes add creator credit, privacy choices, local document-link navigation, Quick Open, image insertion, task-paste preservation, print/PDF preparation, compatibility safeguards, dependency fixes and release-verification preparation.

The initial Linux browser run caught an invalid CRC in a PNG test fixture that macOS tolerated; the fixture was replaced with a checksum-valid PNG, without weakening image validation. Subsequent packaging checks caught target-name validation and archive-name mapping errors in the new checksum utility. The utility now uses explicit platform manifests, matches the actual downloaded archive names, rejects incomplete bundles and has regression coverage for every release target. No failing draft was promoted to a public release.
...
Local verification completed during this update: **26 unit tests, 36 browser tests across Chromium and WebKit, and 12 native macOS file-safety tests passed.** The JavaScript audit reported zero known advisories after the update. The Rust registry audit reported zero vulnerabilities and six unmaintained-dependency warnings after the separately verified GLib backport. The [cross-platform checks for the completed application changes](https://github.com/gopalasubramanium/plainmark/actions/runs/34682944942) also passed: 12 native tests each on Linux and macOS, 9 on Windows, and the Linux GLib regression in both debug and optimized builds. A separate clipboard probe observed no remote HTTP requests from pasted HTML containing remote image and background URLs in Chromium or WebKit. These checks do not establish publisher signing or a complete security audit.

The original [v0.3.3 release run](https://github.com/gopalasubramanium/plainmark/actions/runs/34682945890) passed all checks and built Windows x64, Linux x64, Intel Mac and Apple Silicon Mac packages. Seven downloaded installers/archives matched all four checksum manifests. All eleven artifacts, including the manifests, passed GitHub attestation verification restricted to this repository, the release workflow, `refs/tags/v0.3.3`, source commit `006f03eb0ea13c9e9b768e0217cd3fb2ef2676ef`, and GitHub-hosted runners. The downloaded Mac app launched successfully and displayed version 0.3.3 and the creator explanation.

Final verification also caught parallel builders creating two draft entries for one tag. Their files were checked against the same source identity, consolidated into one complete draft, and the redundant draft was removed after confirming every asset was preserved. The workflow now prepares one draft before the platform matrix, passes its explicit release ID to every builder, and serializes runs for the same tag. Thirteen release-tool tests now cover platform manifests, incomplete bundles, draft reuse, duplicate rejection, notarization and Gatekeeper failures, credential-safe errors, and refusing to overwrite public releases.
